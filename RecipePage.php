...
<body>
	...
	<div class="form-container">
		<form action="RecipePage.php" method="get">
	...
	<?php
		...
		foreach ($json_object['hits'] as $hit) {
			...
			echo '<button type="submit" name="add' . $idnum . '">Buy Meal</button>';
			echo '<button type="submit" name="save' . $idnum . '">Save Recipe</button>';
			echo '</form>';
	...
	<?php 
		...
		for ($i = 0; $i < 50; $i++) {
			$add_fun_call='add' . $i;
			$save_fun_call= 'save'. $i;

			if (isset($_POST[$add_fun_call])) {
				// Call the function to add the JSON object to the database
				addToDatabase($i, 'checkout');
			}

			if (isset($_POST[$save_fun_call])) {
				// Call the function to add the JSON object to the database
				addToDatabase($i, 'saved_recipes');
			}

		}

		function addToDatabase($button_id, $table) {
			...
			foreach ($json_object['hits'] as $hit) {
				if($idnum == $button_id) {
					...
					$sql = "INSERT INTO " . $table . " (user_id, recipe_name, recipe_url, image_url) VALUES ('" . $_SESSION["user_id"] . "', '" . $label . "', '" . $rec_url . "', '" . $img_url . "')";
